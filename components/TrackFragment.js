...
const mapStyle = [
  ...
  },
  {
    ...
    stylers: [{ visibility: "on" }, { color: "#19222a" }, { lightness: 16 }],
  },
  {
    featureType: "all",
  ...
  },
  {
    ...
    stylers: [{ color: "#19222a" }, { lightness: 20 }],
  },
  {
    featureType: "administrative",
  ...
  },
  {
    featureType: "road",
    elementType: "geometry.fill",
    stylers: [{ color: "#d7dee5" }],
  },
  {
    featureType: "road.highway",
    elementType: "geometry.fill",
    stylers: [{ color: "#3f4c5a" }, { lightness: "52" }, { weight: "1" }],
  },
  {
    featureType: "road.highway",
    elementType: "geometry.stroke",
    stylers: [{ color: "#d7dee5" }, { lightness: 29 }, { weight: 0.2 }],
  },
  {
    featureType: "road.arterial",
    elementType: "geometry",
    stylers: [{ color: "#d7dee5" }, { lightness: 18 }],
  },
  {
    featureType: "road.arterial",
    elementType: "geometry.fill",
    stylers: [{ color: "#d7dee5" }, { lightness: "14" }],
  },
  {
    featureType: "road.arterial",
    elementType: "geometry.stroke",
  ...
  },
  {
    ...
    elementType: "geometry",
    stylers: [{ color: "#d7dee5" }, { lightness: 16 }],
  },
  {
    featureType: "road.local",
    elementType: "geometry.fill",
  ...
  },
  {
    featureType: "transit",
    elementType: "geometry",
    stylers: [{ color: "#19222a" }, { lightness: 19 }],
  },
  {
    featureType: "water",
    elementType: "all",
...
const nextPassErrorStr =
  "Could not determine next EQUiSat pass over this location. Try again later.";
...
export default class TrackFragment extends React.Component {
    ...
    _this.serverRequest = axios
      ...
      .then(function (result) {
        ...
        if (isUserLoc) {
          ...
          setTimeout(function () {
            _this.searchLocMarker.showCallout();
          }, 1000);
        }
      })
  ...
  _getLocationAsync = async () => {
    ...
    if (status !== "granted") {
      ...
      setTimeout(function () {
        _this.setState({ locErrorSnackbarVisible: false });
      }, 5000);
    }
  ...
  _getGeocodeLatLong = async () => {
    ...
    if (locations.length == 0) {
      ...
      var _this = this;
      setTimeout(function () {
        _this.setState({ searchErrorSnackbarVisible: false });
      }, 5000);
    } else {
  ...
  setOrbitPathCoords(_this) {
    ...
    for (var i = 0; i < orbitLines[1].length; i++) {
      satCoords = [
        ...satCoords,
        { latitude: orbitLines[1][i][0], longitude: orbitLines[1][i][1] },
      ];
    }
    for (var i = 0; i < orbitLines[2].length; i++) {
      satCoords2 = [
        ...satCoords2,
        { latitude: orbitLines[2][i][0], longitude: orbitLines[2][i][1] },
      ];
    }
  ...
  componentDidMount() {
    ...
    this.satUpdateAsyncID = setInterval(function () {
  ...
  showUserLoc() {
    ...
    var _this = this;
    setTimeout(function () {
      _this.userLocMarker.showCallout();
    }, 1000);
  }
  ...
  showCalloutText(userLoc) {
    var isError = userLoc
      ? this.state.userNextPassError
      : this.state.searchNextPassError;
    var nextPass = userLoc
      ? this.state.userNextPass
      : this.state.searchNextPass;
    if (isError) {
      ...
      return (
        <View style={{ flex: 1 }}>
          ...
          <Text>
            Rise Time: {this.printDate(new Date(nextPass.rise_time * 1000))}
          </Text>
          <Text>Rise Azimuth: {nextPass.rise_azimuth.toFixed(2)}°</Text>
          <Text>Max Alt.: {nextPass.max_alt.toFixed(2)}°</Text>
          <Text>
            Max Alt. Time:{" "}
            {this.printDate(new Date(nextPass.max_alt_time * 1000))}
          </Text>
          <Text>Set Azimuth: {nextPass.set_azimuth.toFixed(2)}°</Text>
          <Text>
            Set Time: {this.printDate(new Date(nextPass.set_time * 1000))}
          </Text>
          <TouchableHighlight
            style={{ padding: 5 }}
            onPress={() => console.log("HIGHLIGHTS")}
  ...
  registerNumber(phoneNumber) {
    ...
    axios
      ...
      .then(function (result) {
        _this.setState({
          notifyStatusSnackbarText: result.data.success
            ? "Successfully registered for SMS notifications"
            : "Error registering for SMS notifications",
        });
        _this.setState({ notifyStatusSnackbarVisible: true });
        setTimeout(function () {
          _this.setState({ notifyStatusSnackbarVisible: false });
        }, 5000);
      ...
      .catch(function (error) {
        ...
        _this.setState({
          notifyStatusSnackbarText: "Error registering for SMS notifications",
        });
        _this.setState({ notifyStatusSnackbarVisible: true });
        setTimeout(function () {
          _this.setState({ notifyStatusSnackbarVisible: false });
  ...
  subscribeSMSNotifications(phoneNumber) {
    ...
    axios
      ...
      .catch(function (error) {
        _this.setState({
          notifyStatusSnackbarText: "Error contacting SMS notification server",
        });
        _this.setState({ notifyStatusSnackbarVisible: true });
        setTimeout(function () {
          _this.setState({ notifyStatusSnackbarVisible: false });
  ...
  render() {
    return (
      <View style={styles.container}>
        <DialogComponent
          ref={(dialogComponent) => {
            this.subscribeDialog = dialogComponent;
          }}
          title={
            <DialogTitle
        ...
        <ActionButton
          ...
          renderIcon={(active) =>
            active ? (
              <Icon name="crosshairs-gps" style={styles.actionButtonIcon} />
            ) : (
              <Icon name="crosshairs-gps" style={styles.actionButtonIcon} />
            )
          }
        ...
        />
        <SnackBar
          visible={this.state.userLocErrorSnackbarVisible}
          textMessage="Can't get location: Permission Denied"
          actionHandler={() => {
            this.setState({ userLocErrorSnackbarVisible: false });
          }}
          actionText="OK"
        />
        <SnackBar
          visible={this.state.searchErrorSnackbarVisible}
          textMessage="No results for location"
          actionHandler={() => {
            this.setState({ searchErrorSnackbarVisible: false });
          }}
          actionText="OK"
        />
        <SnackBar
          visible={this.state.notifyStatusSnackbarVisible}
          textMessage={this.state.notifyStatusSnackbarText}
        />
      </View>
    );
  }
